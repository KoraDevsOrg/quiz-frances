export class TimeAttackModule {
  constructor(words, storageService, audioService) {
    ...
    this.wordFrEl = document.getElementById("taWordFr");
    this.wordPhoneticEl = document.getElementById("taWordPhonetic");
  ...
  updateBestScoreUI() {
    this.bestScoreText.textContent = this.storage.getTimeAttackBest();
  }
  ...
  nextQuestion() {
    ...
    this.currentQ = { target, correct: target.meaning, options };

    this.wordFrEl.textContent = target.word;
    this.wordPhoneticEl.textContent = target.phonetic;
  ...
  _endGame() {
    ...
    const total = this.score + this.mistakes;
    ...
    const isNew = this.storage.setTimeAttackBest(this.score);
    if (isNew && this.score > 0) {
      this.newRecordMsg.style.display = "block";
      this.audio.speakFrench("Félicitations ! Nouveau record !");
    } else {
